...
class PLMoCo(nn.Module):
    ...
    def forward(self, im_q, im_k, mask_q):
        ...
        # reshape and normalize for cosine similarity
        ...
        # positive logits: (BHW)x1
        l_pos = einops.reduce(torch.bmm(q, k.transpose(1, 2)) * mask_q, 'b hw_q hw_k -> (b hw_q) 1', 'sum')
        # negative logits: (BHW)xK
        l_neg = torch.mm(einops.rearrange(q, 'b hw_q c -> (b hw_q) c'), self.queue.clone().detach())
        ...
        # dequeue and enqueue
        k = einops.rearrange(k, 'b hw_k c -> (b hw_k) c')
        sample_idx = np.random.choice(k.size(0), self.K // 64, replace=False)
        self._dequeue_and_enqueue(k[sample_idx])

        loss = self.criterion(logits, labels)
        nonzero_idx = logits[:, 0] > 0
        acc = accuracy(logits[nonzero_idx], labels[nonzero_idx].unsqueeze(1))[0]

...
from .builder import PLMoCoPlus, PatchLevelEncoder
...
class Worker(BasicWorker):
    ...
    def _init_additional_meters(self):
        loss_moco = AverageMeter('Loss(MoCo)', ':.4e')
        acc_moco = AverageMeter('Acc(MoCo)', ':6.2f')
        loss_plmoco = AverageMeter('Loss(PLMoCo)', ':.4e')
        acc_plmoco = AverageMeter('Acc(PLMoCo)', ':6.2f')
        return [loss_moco, acc_moco, loss_plmoco, acc_plmoco]
    ...
    def _forward(self, data):
        ...
        with torch.cuda.amp.autocast(enabled=self.args.fp16):
            loss, loss_moco, acc_moco, loss_plmoco, acc_plmoco = self.model(im_q=images[0], im_k=images[1], mask_q=mask_q)
        # return loss and additional info (as list)
        return loss, [loss_moco, acc_moco, loss_plmoco, acc_plmoco]
...
from .builder import MoCo, ResNetEncoder


class Worker(BasicWorker):
    def _create_model(self):
        model = MoCo(
            ResNetEncoder, self.args.moco_dim,
            self.args.moco_k, self.args.moco_m, self.args.moco_t)
    ...
    def _forward(self, data):
        ...
        with torch.cuda.amp.autocast(enabled=self.args.fp16):
            loss, acc = self.model(im_q=images[0], im_k=images[1])
